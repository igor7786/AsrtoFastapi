  ...
  const mutation = useMutation(
    {
      mutationFn: async () => {
        pending.set(true);
        const parsed = inputLoginSocialSchema.safeParse({ provider });
        if (!parsed.success) {
          toast(
      ...
      onError: async (error) => {
        toast(
          <div className="flex items-center gap-2">
            <X className="text-red-500" />
            <span>{error.message ?? 'Failed to login!.'}</span>
          </div>,
          {
            id: idToast,
            duration: 1000,
    ...
    >
      {mutation.isPending ? (
        ...
        <>
          ...
          <span>Login with {providerCapitalized}</span>
        </>
      ) : provider === 'github' ? (
        <>
          <svg
          ...
          Login with {providerCapitalized}
        </>
      ) : null}
    </Button>
  );
