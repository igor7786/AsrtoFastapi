...
"""
This script checks the versions of packages in requirements.txt against the latest version available on PyPI.

The script reads the requirements.txt file and checks each package against the latest version available on PyPI.
The package name and version are extracted from the requirements line with the parse_requirement_line function.
The get_latest_version function checks the latest version of the package on PyPI and returns it.
The results are printed to the console.

The script uses the requests library to make a GET request to PyPI and get the latest version of each package.
The packaging library is used to compare the versions.
"""
...
def parse_requirement_line(line: str) -> Optional[Tuple[str, str, str]]:
    """Parse a requirement line into (package, version, comment). Only supports '==' specifier."""
    line = line.strip()
    if not line or line.startswith("#"):
        return None

    # Extract package and version for '==' only
    if "==" in line:
        ...
        print(f"Skipping unsupported requirement line: {line}")
    return None
            ...
            for line in f:
                ...
                try:
                    if version.parse(ver) < version.parse(latest):
                        ...
                        new_line = (
                            f"{pkg}=={latest}{' ' + comment if comment else ''}\n"
                        )
...
def main():
    ...
    outdated = [
        pkg
        for pkg, (_, _, status, _) in updates.items()
